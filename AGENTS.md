...
- Aplicar `supabase/self-host/migrate-pack-size.sql` (packs por producto: `products.pack_size` — ej: sandwiches de miga de a 6 por gusto. Cuando está seteado, `products.price` es el **precio del paquete** y la unidad derivada = `price/pack_size`; qty debe ser múltiplo de `pack_size` en micrositio (stepper de a N), `resolveOrderPricing` rechaza 400 si no, y Mostrador/Mesas (UI + `pos/order`/`pos/consumicion` server) también lo exigen).
