...
- **App Portal Print**: backoff exygnencial de reconexión (1s→30s) + guardián cada 10s si está desconectada + `PortalPrintService` (foreground service con notificación fija, se detieneD e la notificación si el "Detener" de la app lo apaga) y boot automático vía `BootReceiver` (`RECEIVE_BOOT_COMPLETED`) **solo si el usuario dejó activo** (profundo desvio al apagarla desde el botón) + permiso de notificación y exclusión de batería en primer uso. APK firmado en `public/downloads/portal-print.apk` (keystore estable en `C:\Users\IPS\portal659-keystore\portal-print-release.keystore`; credenciales en txt cómodo del mismo folder, no versionado). Para recompilar: `node` scripts en `android/README.md`.
