...
- **Llave maestra MP (`MP_ENABLED`)**: on/off global de todo Mercado Pago (`isMpEnabled()` en `src/lib/mp-oauth.ts`). Solo con `MP_ENABLED=1` operan: card de conexión (`MpConnectCard` la oculta consultando `GET /api/mp/status`), `GET /api/mp/connect` + callback OAuth (403/redirect si está apagado), `POST /api/payments` y rama MP de `/api/subscriptions/pay` (cae a transferencia manual). En dev va en `.env.local`; **en prod está encendida** (`MP_ENABLED=1` la escribe el `deploy.yml` desde sep-2026; para apagar toda la funcionalidad MP sin tocara código basta volverla `0`). Día del encendido: credenciales reales en secrets de GitHub (listas) + Redirect URL y Webhook en la app de MP (`/api/mp/oauth/callback` y `/api/webhooks/mercadopago`) + aplicar `migrate-mp-oauth.sql` contra el contenedor si falta.
...
- **Agente PC (Windows)**: app Electron en `services/print-agent/` con **ventana de configuración + bandeja del sistema** (reemplaza al agente CMD). `src/main.js` (proceso principal: ventana, bandeja, IPC, autoarranque vía `HKCU\...\Run` sin admin), `src/relay.js` (WebSocket→TCP), `src/renderer/*` (UI). Config en `%APPDATA%\portal-print-agent\agent.config.json`. **Distribución en `.zip`** (carpeta, no portable: el wrapper NSIS portable disparaba falsos positivos AVG `IDP.HEUR.26`): `npm run build:win:zip` → `dist/portal-print-agent.zip` (recorta `locales/` a es/en); el zip **no se versiona** y se publica con `docker cp` al volumen `/app/uploads/downloads/` (link en `dashboard-gastro.tsx` con `?v=` de cache-bust, hoy `?v=3`). AVG/Avast puede marcar el `.exe` interno como falso positivo heurístico: documentado en README y `manuales/impresora` (restaurar + excluir + reportar en `https://www.avg.com/en-ww/report-false-positive`). Legacy headless: `agent.mjs` (sin deps, se usa en `test/e2e.mjs`).
...
- **Piloto Bot de WhatsApp** (`docs/piloto-whatsapp-bot.md`): bot que toma pedidos por WhatsApp (add-on Gestión). Motor whatsmeow (Go) en el relay `services/wa-bot/relay` embebido en el APK "Portal Wa Link" (`portal-wa-link/`, binario como `jniLibs/arm64-v8a/librelay.so`); cerebro Node `services/wa-bot/` en el VPS (servicio `wabot`, WS `/wa` en nginx). El bot **solo responde a inbound, nunca inicia conversaciones**; kill switch por comercio (`vendor_wa_bots.enabled`). **Anti-ban integrado**: pacing humano (`WA_REPLY_DELAY_*`, `WA_REPLY_GAP_MS`), indicador de tipeo (cerebro→relay `SendChatPresence`), rate limits por comercio en `src/limits.mjs` (`WA_MAX_MSG_PER_HOUR`/`DAY`, `WA_MAX_NEW_CHATS_PER_HOUR` → handoff al dueño), plantilla de menú variada (`src/menu.mjs`), backoff de re-pareo en relay (`WA_REPAIR_BACKOFF_MAX`), telemetría `/health` + logs `[ban-risque]`. APK `public/downloads/portal-wa-link.apk` con cache-bust `?v=N` (hoy `?v=12`); rebuild: `go build` del relay → `gradlew assembleRelease` en `portal-wa-link/`.
...
- Aplicar `supabase/self-host/migrate-pack-size.sql` (packs por producto: `products.pack_size` — ej: sandwiches de miga de a 6 por gusto. **Precio es del PAQUETE**; la línea nunca suma unidades redondeadas — el dinero se computa pack-native (`packPrice × N packs`) y los items persistidos guardan `price` = pack completo + `pack_size`, con `orderLineTotal()` (en `src/lib/order-line.ts` junto a `cartLineTotal()` del carrito) en todos los displays. qty múltiplo de N — micrositio (stepper), carrito, Mostrador/Mesas (UI + `pos/order`/`pos/consumicion`/`tables/close` server), y `resolveOrderPricing` rechaza 400 si no. Volumen trabaja con unidad FULL PRECISION para no arrastrar decimales).
