...
export function StickyWhatsApp({ url, isService, isRetail, isUrgent, urgentUrl, urgentLabel }: StickyWhatsAppProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-sm border-t border-border px-4 py-3 sm:hidden">
      <div className="flex gap-2">
        <a
          ...
          className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-whatsapp text-white py-3 text-sm font-medium hover:bg-whatsapp-dark transition-all active:scale-95"
        >
          ...
          {isService ? "Consultar" : isRetail ? "Comprar por WhatsApp" : "Pedir por WhatsApp"}
        </a>
        {isUrgent && urgentUrl && (
          <a
            href={urgentUrl}
